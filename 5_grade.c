...
void my_handler(int nsig)
{
    ...
    sem_destroy(&db->write_semaphore);
...
int main(int argc, char *argv[])
{
    ...
    db->num_r = 0;
    // Создание неименованного семафора для синхронизации процессов-писателей
    if (sem_init(&db->write_semaphore, 1, 1) ==
        -1)
    { // значение 1 указывает, что семафор будет использоваться для синхронизации процессов
        perror("sem_init readers_sem");
        return 1;
    }
